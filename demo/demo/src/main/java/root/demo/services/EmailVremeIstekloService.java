...
@Service
public class EmailVremeIstekloService implements JavaDelegate{
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		String autorMail = (String) execution.getVariable("istekloVremeMail");
		...
		//root.demo.model.User korisnik = userRepository.findByEmail(email);	
		       // smtpMailSender.send(userDTO.getEmail(), "Test mail from Spring", "http://localhost:4200/auth/confirmation"+userDTO.getId());
		        ...
		        try {
		            ...
		            message.setRecipients(Message.RecipientType.TO,InternetAddress.parse(autorMail));	
		      //      message.addRecipients(Message.RecipientType.CC,InternetAddress.parse(autorMail));	
		            message.setSubject("Isteklo vreme za recenziju");
		            message.setText("Postovani,"
		                    + "\n\n propustili ste vasu sansu za recenziju!"+"\n \n");
